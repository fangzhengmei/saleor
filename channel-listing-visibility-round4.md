# Saleor Channel 可见性深度分析（第四轮 - 修正版）

## 错误修正说明

上一轮分析中的边界示例存在两处不合法的 GraphQL 查询：

1. ❌ `filter: { product: "UHJvZHVjdDox" }` - `ProductVariantFilter` 没有 `product` 过滤字段
2. ❌ `filter: { price: { gte: 150 } }` - `ProductVariantFilter` 没有 `price` 过滤字段

### `ProductVariantFilter` 实际支持的字段

**代码位置**：`saleor/graphql/product/filters/product_variant.py:697`

```python
class ProductVariantFilter(MetadataFilterBase):
    search = django_filters.CharFilter(method="product_variant_filter_search")  # 搜索名称/SKU/产品名
    sku = ListObjectTypeFilter(input_class=graphene.String, method=filter_sku_list)  # 按SKU列表过滤
    is_preorder = django_filters.BooleanFilter(method=filter_is_preorder)  # 按是否预售过滤
    updated_at = ObjectTypeFilter(input_class=DateTimeRangeInput, method=filter_updated_at_range)
```

### `ProductVariantWhere` 实际支持的字段

**代码位置**：`saleor/graphql/product/filters/product_variant.py:720`

```python
class ProductVariantWhere(MetadataWhereFilterBase):
    ids = GlobalIDMultipleChoiceWhereFilter(method=filter_by_ids("ProductVariant"))  # 按ID过滤
    sku = ObjectTypeWhereFilter(input_class=StringFilterInput, method="filter_product_sku")  # 按SKU过滤
    updated_at = ObjectTypeWhereFilter(input_class=DateTimeRangeInput, method="filter_updated_at")
    attributes = ListObjectTypeWhereFilter(input_class=AssignedAttributeWhereInput, method="filter_attributes")
```

### `resolve_product_variants` 的 `product_id` 参数

**代码位置**：`saleor/graphql/product/resolvers.py:193`
...
def resolve_product_variants(
    ...
    if product_id:
        qs = qs.filter(product_id=product_id)  # 内部使用，不对外暴露
```

**关键点**：`product_id` 参数存在但**仅用于内部调用**（由 `product { productVariants }` 字段传入 `root.node.pk`），**没有在 GraphQL schema 中暴露**给外部调用者。
...
**结果**：✅ 产品 P 返回，变体列表包含 V2 (price=200)，但不包含 V1 (price=100)
- 管理员豁免 `visible_in_listings` 检查
- 产品级价格过滤：V2 的价格 `200 >= 150` 匹配，V1 不匹配但 V2 匹配，所以产品返回
- 变体列表返回该产品的所有变体（因为 `productVariants` 内部没有价格过滤）
